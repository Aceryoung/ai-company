    ...
    <div className="min-h-dvh bg-[#E0F2F1] px-4 py-6">
      <div className="max-w-3xl mx-auto space-y-4">
        <div className="bg-white rounded-2xl p-5 shadow-sm">
          <h1 className="text-xl font-bold text-gray-900 mb-1">구글 시트 데이터 가져오기</h1>
          <p className="text-sm text-gray-500">아래 {IMPORT_DATA.length}건을 지출 내역으로 일괄 등록합니다.</p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                ...
                <th className="text-left px-4 py-3 text-gray-500 font-medium hidden md:table-cell">메모</th>
              </tr>
            </thead>
            <tbody>
              {IMPORT_DATA.map((row, i) => (
                <tr key={i} className="border-b border-gray-50 last:border-0">
                  <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{row.transaction_date}</td>
                  ...
                  <td className="px-4 py-3 text-right text-red-500 font-medium whitespace-nowrap">
                    {row.memo.includes("USD") ? `$${row.amount}` : `₩${row.amount.toLocaleString()}`}
                  </td>
                  <td className="px-4 py-3 text-gray-400 text-xs hidden md:table-cell truncate max-w-[200px]">{row.memo}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        >
          {loading ? "가져오는 중..." : `${IMPORT_DATA.length}건 일괄 가져오기`}
        </button>
      </div>
    </div>
  );

...
export default function NewQuoteForm({ clients, projects, existingNumbers }: Props) {
  ...
  const [quoteNumber, setQuoteNumber] = useState(generateQuoteNumber(existingNumbers));
  const [validityDays, setValidityDays] = useState("14");
  const [productionPeriod, setProductionPeriod] = useState("약 3-4주");
  ...
  return (
    <div className="flex flex-col min-h-dvh bg-[#E0F2F1]">
      <header className="flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-100">
        ...
        <h1 className="text-base font-bold text-gray-900">견적서 작성</h1>
      </header>
      ...
      <div className="flex-1 px-4 py-4 space-y-4 pb-32 md:max-w-3xl md:mx-auto md:w-full">
            ...
            <div key={item.key} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                  ...
                  {item.is_section && (
                    <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded">구분선</span>
                  )}
                </div>
                <button
                  type="button"
              ...
              <input
                ...
                placeholder={item.is_section ? "구분 제목 (예: 기본 제공 항목)" : "항목명"}
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-[#26A69A]"
              />
              ...
              {!item.is_section && (
                <>
                  <textarea
                    ...
                    placeholder="세부 설명 (줄바꿈으로 여러 줄 입력)"
                    rows={2}
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="space-y-1">
                      ...
                      <input
                        ...
                        onChange={(e) => updateItem(item.key, { unit_price: fmtNum(e.target.value) })}
                        placeholder="0"
                        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-900 focus:outline-none focus:border-[#26A69A] text-right"
                      />
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="space-y-1">
                      ...
                      {item.unit_price && item.quantity ? (
                        ...
                        <input
                          type="text"
                          value={item.amount}
                          onChange={(e) => updateItem(item.key, { amount: fmtNum(e.target.value) })}
                          placeholder="직접 입력"
                          className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-900 focus:outline-none focus:border-[#26A69A] text-right"
                        />
                      )}
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-gray-700">금액 대신 표시할 텍스트 (예: 별도, 미포함)</label>
                    <input
                      type="text"
                      value={item.price_label}
                      onChange={(e) => updateItem(item.key, { price_label: e.target.value })}
                      placeholder="비워두면 금액 표시"
                      className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-[#26A69A]"
                    />
                  </div>
                  ...
                  <div className="text-right text-xs text-gray-700">
                    ...
                    <span className="font-semibold text-gray-700">
                      {item.price_label
                        ? item.price_label
                        : itemTotal(item) != null
                        ? `${(itemTotal(item) ?? 0).toLocaleString()}원`
                        : "-"}
                    </span>
                  </div>
                </>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-2">
          ...
          <div className="flex justify-between text-sm border-t border-gray-100 pt-2">
            <span className="font-semibold text-gray-900">최종 견적(VAT 포함)</span>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-2">
          ...
          <textarea
            ...
            placeholder="견적서 하단에 표시될 참고사항을 입력하세요"
            rows={4}
            className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-[#26A69A] resize-none"
          />
        </div>

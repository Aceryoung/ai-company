...
// 부서별 보유 스킬 (AI 프롬프트 주입용 — 직원이 업무 시 활용)
const DEPT_SKILLS: Record<string, string> = {
  시장조사: '🔍 웹 검색(시장 트렌드·경쟁사), 🌐 웹 수집(URL 크롤링), ⚔️ 경쟁사 분석(비교·대안 분석), 🎯 고객 리서치(페르소나·니즈), 🧠 심리 분석(소비자 행동 패턴), 📡 30일 트렌드(SNS 트렌드 분석)',
  영업:     '📧 메일 검색(고객 문의), 📋 노션 검색(제안서·견적서), 🗄️ DB 조회(클라이언트·프로젝트), 🎣 잠재고객 발굴(아웃바운드 리서치), 📩 콜드메일(접근용 이메일), 🗂️ 영업 자료(배틀카드·세일즈 콘텐츠), 💲 가격 전략(가격 책정 최적화)',
  기획:     '📝 노션 작성(PRD·스펙 문서), 🎨 피그마(와이어프레임·UI), 💡 브레인스토밍(아이디어 발산), 📰 콘텐츠 전략(채널 전략), 🎯 제품 마케팅(ICP·포지셔닝), 📋 마케팅 플랜(AARRR 기반 로드맵)',
  검수:     '🔎 코드 리뷰(품질·보안 리뷰), 🛡️ 보안 리뷰(RLS·env·인젝션), 🧪 테스트 실행(npm test·lint), 📏 AI 평가(모델 출력 품질 벤치마크), 🕸️ 코드 그래프(의존성 분석), 🔓 취약점 스캔(보안 AI 탐지)',
  개발:     '📖 코드 읽기(소스 분석), ✏️ 코드 작성(파일 생성), 🔧 코드 수정(정밀 편집), ⚡ 커맨드 실행(npm·git·CLI), 🤖 AI 추론(LLM 엔진 호출), 🕸️ 코드 그래프(구조 시각화), 🔐 코드 보안(취약점 자동 탐지)',
  배포:     '🚀 Vercel 배포(프로덕션), 🔀 GitHub PR(PR 생성·머지), 📦 빌드 확인(npm run build), 🛟 장애 대응(폴백·재시도), ⚡ 캐시 관리(CDN 최적화), 🛡️ CI 보안 스캔(PR마다 자동 스캔)',
  고객소통: '📨 메일 발송(고객 응대), 💬 카카오 메모(카카오톡 전송), 📋 피드백 기록(노션 DB), 🚪 온보딩 설계(신규 고객 플로우), 📮 이메일 시퀀스(자동 이메일), 🔒 이탈 방지(고객 이탈 예측), 👥 커뮤니티(고객 커뮤니티 운영)',
  정산:     '🗄️ DB 조회(거래·정산 데이터), 📊 엑셀 생성(정산표·세금계산서), 📄 PDF 생성(청구서·리포트), 💹 매출 운영(파이프라인·전환율), 📈 분석 추적(GA4·이벤트 트래킹), 🎯 기여도 분석(채널별 매출 기여도)',
  회고:     '📖 회고 작성(스프린트 회고 문서), 📈 리포트 생성(시각화 리포트), 🧠 회고 분석(KPT·4L 프레임워크), 🔁 패턴 분석(반복 실수·안티패턴 탐지), 🏛️ 마케팅 리뷰(전략 합의·우선순위)',
  운영:     '📋 서버 로그(Vercel 런타임 로그), 🗄️ DB 로그(Supabase 쿼리 로그), 🔒 모니터링(서버 상태·헬스체크), ☁️ AI 프로바이더(상태·전환 관리), 💰 비용 관리(API 비용 추적), 🩺 헬스 체크(인프라 진단)',
  비서:     '📅 일정 관리(스케줄·리마인더), 🔍 문서 검색(전사 문서·회의록), ✉️ 메일 초안(메일 작성), 📌 플랜 관리(프로젝트 진행상황), 🧠 컨텍스트(대화 맥락·이전 결정사항 기억)',
  레포:     '🔗 GitHub 조회(레포·커밋·PR 현황), 📂 Git 명령(git log·diff·status), 🔎 코드 분석(코드베이스 구조), 🐙 GitHub 스킬(이슈·PR·액션 자동화), 🕸️ 코드 그래프(아키텍처·의존성)',
  채용:     '📊 조직 분석(부서 구조·인력 현황), 🏗️ 부서 제안(신규 부서·역할 신설), 🧩 인력 검토(충원 필요성·적정 인원), 🤝 A2A 에이전트(에이전트 간 협업 설계)',
  마케팅:   '📰 콘텐츠 전략(채널 전략), 🎯 제품 마케팅(포지셔닝·메시징), 📋 마케팅 플랜(12개월 로드맵), 📈 분석 추적(GA4·이벤트 트래킹), 📡 30일 트렌드(SNS 트렌드)',
  경영:     '📊 전략 분석(사업 전략·KPI), 🏛️ 의사결정(부서간 조율·컨펌), 📋 보고서 종합(각 부서 보고 종합)',
}
...
export async function POST(req: NextRequest) {
  ...
  if (body.action === 'meeting') {
    ...
    const leaderList = leaders.map(l => {
      const skills = DEPT_SKILLS[l.dept]
      return `- ${l.name} (${l.dept} ${l.role})${skills ? ` — 보유 스킬: ${skills.slice(0, 80)}…` : ''}`
    }).join('\n')
...
발언 시 본인 보유 스킬 중 활용할 수 있는 것이 있으면 [스킬명]으로 언급하세요.
...
평소 말버릇: "${speech}"
${skills ? `\n■ 보유 스킬: ${skills}\n  → 업무에 적합한 스킬을 활용하고, 사용한 스킬은 [스킬명]으로 표시하세요.` : ''}

■ 대표님이 내린 업무 지시: "${taskDesc}"
${context ? `■ 이전 단계 결과:\n${context}\n` : ''}

■ 출력 규칙:
1. 첫 줄: [진행] 작업 시작 한마디 (사용할 스킬 언급)
2. 본문: 작업 결과 (3~5문장, 구체적 수치 포함, 어떤 스킬로 수행했는지 표시)
3. 마지막 줄: [완료] 완료 보고
...
1문장으로 결과 보고. 구체적 수치 포함, 존댓말, 이모지 1개, 미래 약속 금지.`
  ...
  const skillList = DEPT_SKILLS[dept] || ''
...
■ 보유 스킬 (업무 시 적극 활용, 사용한 스킬은 [스킬명]으로 표시)
${skillList}
